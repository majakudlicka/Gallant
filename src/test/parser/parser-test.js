...
describe('Parser', () => {
	...
	it('should parse a new variable declaration using a different greeting token (word)', () => {
	...
	// Only valid for re-assignment of already delcared variable
	...
	// Emoji that does not have any special meaning
	...
	it('should parse a null constant', () => {
	...
	it('should parse the boolean constant "true"', () => {
	...
	it('should parse the boolean constant "false"', () => {
	...
	it('should parse an assignment to an array', () => {
		const parser = new Parser(
			'hi arr = [1, 2, 3]'
		);
		const node = parser.parse();
	...
	it('should parse an assignment to a map', () => {
		const parser = new Parser(
			'hi myMap = {a = 1, b = 2}'
		);
		const node = parser.parse();
		assert.equal(true, node.isAssignmentNode());
		const { value, symbol } = node;
		assert.equal(true, value.isMapNode());
		assert.equal(true, symbol.isSymbolNode());
	});

	it('should parse a map accessor', () => {
		const parser = new Parser(
			'myMap@x'
		);
		const node = parser.parse();
		...
		const { ref, key } = node;
		assert.equal(true, ref.isSymbolNode());
		assert.equal(true, key.isSymbolNode());
	});
